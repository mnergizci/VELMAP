...
for igf = 1:gpspar.ngpsfile
  ...
  if gpspar.vel3d{igf}==0
    ...
    [lon,lat,ve,vn,stde,stdn,coven,staid] = textread(filename,'%f %f %f %f %f %f %f %s','commentstyle','shell');
  else
    gps(igf).invenu=[1 1 1];
    [lon,lat,ve,vn,vu,stde,stdn,stdu,coven,coveu,covnu,staid] = textread(filename,'%f %f %f %f %f %f %f %f %f %f %f %s','commentstyle','shell');
  end
